...
let simpleSelectColumn = 'id, classification_id classificationId, name, type, shortcut_key shortcutKey, global_shortcut_key globalShortcutKey, `order`'
 ...
 * @param reuseId
 */
function batchAdd(classificationId: number, itemList: Array<Item>, reuseId: boolean = false) {
	// 返回信息
...
function update(item: Item) {
	...
	let sql = `UPDATE ${tableName} 
            ...
            WHERE id = ?`
	// 运行
	return db.prepare(sql).run(item.name, JSON.stringify(item.data), item.shortcutKey, item.globalShortcutKey ? 1 : 0, item.id).changes > 0
}
 ...
 * @param toId
 */
function updateOrder(fromIdList: Array<number>, toClassificationId: number, newIndex: number | null) {
	// 查询来源项目
	...
	if (fromItemList.length > 0) {
		...
		for (const item of fromItemList) {
			if (item.classificationId !== toClassificationId && !fromClassificationIdList.includes(item.classificationId)) {
...
export { init, list, add, batchAdd, update, del, selectById, selectByIdList, deleteByClassificationId, updateOrder, updateData }
